...
CORS(app,
    origins=['https://senadi.vercel.app', 'http://127.0.0.1:3000'],
...
def register():
    ...
    try:
        ...
        cursor.execute(
            ...
            (user_id, username, email, hash_password, 'user',)
        )
        conn.commit()
...
@app.route('/chat/new', methods=['POST', 'OPTIONS'])
@jwt_required()
def create_chat():
    if request.method == 'OPTIONS':
    ...
    try:
        ...
        insert_query = f"INSERT INTO chats ({', '.join(insert_columns)}) VALUES ({placeholders}) RETURNING id"
...
def admin_chats():
    try:
        ...
        cursor.execute("""
            SELECT 
                c.id, c.user_id, c.created_at, c.verified, u.username,
                (SELECT COUNT(*) FROM messages m WHERE m.chat_id = c.id) as message_count,
                (SELECT MAX(m.created_at) FROM messages m WHERE m.chat_id = c.id) as last_message_time,
                (SELECT message FROM messages m WHERE m.chat_id = c.id AND m.sender = 'bot' ORDER BY m.created_at DESC LIMIT 1) as last_bot_message,
                (SELECT message FROM messages m WHERE m.chat_id = c.id AND m.sender = 'user' ORDER BY m.created_at DESC LIMIT 1) as last_user_message
        ...
        for chat in chats:
            ...
            if chat['message_count'] is not None:
                ...
                print(f"Jumlah message terhitung: {chat['message_count']}")
            else:
                chat['message_count'] = 0
                print("Tidak ada message terhitung")
...
def admin_delete_file_github(filename):
    ...
    try:
        ...
        try:
            ...
            print("File tidak ada di database")
...
def admin_get_chat(chat_id):
    try:
        ...
        chat = cursor.fetchone()
        chat = dict(zip(
            ['id', 'user_id', 'created_at', 'verified', 'username'],
            chat
        ))
        ...
        messages = [dict(zip(
            ['id', 'message', 'sender', 'created_at', 'is_corrected', 'is_correction'],
            row
        )) for row in messages]
        ...
        print(f"Ini hasil chat dari admin get chat: {chat}")
        print(f"Ini hasil messages dari admin get chat: {messages}")
...
def upload_file_github():
    ...
    try:
        ...
        try:
            ...
            print("Sukses masuk ke Github")
        except Exception as e:
            print(f"Gagal upload ke Github karena: {str(e)}")
            return jsonify({"error": f"Gagal upload ke Github karena: {str(e)}"}), 500
        ...
        try:
            ...
            print(f"Gagal masuk ke Supabase karena: {str(db_err)}")
